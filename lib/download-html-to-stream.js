...
function download(src, options) {
  ...
  var months = exports.readIndex(src, options).then(function (months) {
  ...
  return toStream(months)
    ...
    .flatMap(function (month) {
      return exports.readMonth(month, options)
    }, {parallel: options.parallelMonths || 2})
    .filter(filterMessage)
    .map(function (url) {
      // we don't pass options here because a `cache `option
      // is probably only meant for months and indexes
