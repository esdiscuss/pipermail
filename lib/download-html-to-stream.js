...
function download(src, options) {
  ...
  var parallel = options.parallel || 10
  assert(typeof maxMonths === 'number' && !isNaN(maxMonths) && maxMonths >= 1 && maxMonths % 1 === 0 || maxMonths === Infinity, '`months` must be a positive integer or infinity.')
  assert(typeof parallel === 'number' && !isNaN(parallel) && parallel >= 1 && parallel % 1 === 0, '`parallel` must be a positive integer.')
  ...
  dest._transform = function (url, _, callback) {
    ...
    queue.push(complete)
    if (queue.length < parallel) callback()
    else cb = callback
  }
